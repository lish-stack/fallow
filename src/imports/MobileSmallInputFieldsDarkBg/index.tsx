type MobileSmallInputFieldsDarkBgProps = {
  className?: string;
  property1?: "Default" | "input" | "error" | "focus";
};

export default function MobileSmallInputFieldsDarkBg({ className, property1 = "Default" }: MobileSmallInputFieldsDarkBgProps) {
  if (property1 === "focus") {
    return (
      <div className={className || "bg-[#f5f0e8] content-stretch flex items-start px-[10px] py-[8px] relative rounded-[8px] w-[420px]"} data-name="Property 1=focus">
        ...
        <p className="[word-break:break-word] font-['Source_Sans_3:Medium',sans-serif] font-medium leading-[22px] relative shrink-0 text-[#1a1a18] text-[14px] whitespace-nowrap">|</p>
      </div>
    );
  }
  if (property1 === "input") {
